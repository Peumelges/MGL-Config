...
# File: set_zabbix_proxy.sh
# Description:
# This script updates the 'Server=' line in /etc/zabbix/zabbix_proxy.conf
...
dialog --inputbox "Enter the new Zabbix server address:" 8 50 2>/tmp/zabbix_server_input.txt
response=$?
...
if [[ -z "$NEW_SERVER" ]]; then
    dialog --msgbox "No server address entered. No changes made." 6 50
    exit 1
fi
...
# Replace the Server= line with the new value using sed
...
# Replace the ServerActive= line with the new value using sed
sed -i "s/^ServerActive=.*/ServerActive=${NEW_SERVER}/" "$CONFIG_FILE"
